...
process_results_dataframe <- function(df) {
  # add 'subsample_ID' and 'class' to the results dataframe
  df <- merge(df, subset(subsamples_edge_lists, select = c(link_ID, subsample_ID, class)), by = "link_ID")
  
  # add 'subsample_ID' and additional metadata to the results dataframe
  df <- merge(df, subset(subsamples_metadata, select = c(subsample_ID, name, community)), by = "subsample_ID")
...
# Load results
results_types <- read.csv(paste0(path_raw_results, "results_domains.csv")) # types comparison
results_models <- read.csv(paste0(path_raw_results, "results_models.csv")) # models comparison
# results_subset <- read.csv(paste0(path_raw_results, "results_subset.csv")) # subset comparison

# Process the results dataframes
results_types <- process_results_dataframe(results_types)
results_models <- process_results_dataframe(results_models)
# results_subset <- process_results_dataframe(results_subset)
...
calc_metrics <- function(data, metrics=c("roc_auc", "pr_auc", "recall", "precision", "specificity", "f1", "balanced_accuracy", "mcc", "youden_j", "log_loss")) {
  ...
  if("balanced_accuracy" %in% metrics) res_list$balanced_accuracy <- cm$byClass['Balanced Accuracy'] # store the balanced accuracy value
...
df_melt$is_FN <- df_melt$class == "FN"
...
# Define the columns to group by based on the splitting method
if (nested_cv){ # Nested CV
  cols_to_group_by <- c("fold", "network", "type_train", "type_test")
} else {
  cols_to_group_by <- c("network", "type_train", "type_test")
}
...
# Long format
metrics_type_df_long <- metrics_type_df %>%
  pivot_longer(names_to = "metric", values_to = "value", -all_of(cols_to_group_by)) %>%
  mutate(metric = factor(metric, levels = metrices_levels)) # reorder

# Display the plot
if (export){
  write.csv(metrics_type_df_long, file = paste0(path_intermediate_results, "metrics_type_df_long.csv"), row.names = FALSE)
}
...
network_lvl_features <- original_networks_df[!duplicated(original_networks_df$subsample_ID), ] # drop dulicated rows, leave only one row from each network
network_lvl_features <- merge(network_lvl_features, subset(subsamples_metadata, select = c(subsample_ID, name, community)), by = "subsample_ID")
network_lvl_features <- merge(network_lvl_features, features, by = "link_ID") # add features
network_lvl_features <- subset(network_lvl_features, select = -c(link_ID, lower_level, higher_level, weight, class)) # drop unrelevant columns

# Filter network types | keep only ecological
network_lvl_features <- network_lvl_features %>% filter(community %in% c("Plant-Pollinator", "Host-Parasite", "Plant-Seed Dispersers", "Plant-Herbivore"))

if (export){
  write.csv(network_lvl_features, file = paste0(path_intermediate_results, "network_lvl_features.csv"), row.names = FALSE)
}
```
...
# Check if there are columns in cols_keep that are not found in network_lvl_features
# print(cols_keep[!cols_keep %in% colnames(network_lvl_features)])

# keep only these columns
network_lvl_features_pca <- network_lvl_features[, cols_keep]

# Split to x and y, y is 'community'
X <- network_lvl_features_pca %>% select(-c(subsample_ID, name, community))
y <- network_lvl_features_pca$community
...
# print(colnames(network_lvl_features_pca)[!colnames(network_lvl_features_pca) %in% colnames(X)])
...
#### Now for the trunsductive results
...
transductive_results = read.csv(paste0(path_raw_results, 'results_ML_by_single_networks.csv'))
...
compare_models_df <- results_types %>%
...
# Create the plot
compare_models_plot <- ggplot(compare_models_metrics_df, aes(x = metric, y = value, fill = model)) +
  geom_boxplot() +
...
# ROC
compare_models_roc_df <- compare_models_df %>%
  group_by(model) %>%
  nest() %>%
  mutate(roc = map(data, ~ roc(.x$y_true, .x$y_proba, quiet = TRUE)),
         auc = map_dbl(roc, ~ .x$auc), # Extract AUC for each ROC curve
         coords = map(roc, ~ data.frame(tpr = .x$sensitivities, fpr = 1 - .x$specificities))) %>%
  select(-data, -roc) %>%
  unnest(cols = c(coords)) %>%
  ungroup()

# Plot the ROC curves
compare_models_roc_plot <- ggplot(compare_models_roc_df, aes(x = fpr, y = tpr, color = model)) +
  geom_line() +
  geom_abline(linetype = "dashed") +
  labs(title = "ROC Curves by Model and Subsample ID",
       x = "False Positive Rate (1 - Specificity)",
       y = "True Positive Rate (Sensitivity)",
       color = "Model") +
  theme_minimal()

print(compare_models_roc_plot)

# PR
compare_models_pr_df <- compare_models_df %>%
  group_by(model) %>%
  nest() %>%
  mutate(pr = map(data, ~ pr.curve(weights.class0 = .x$y_true, scores.class0 = .x$y_proba, curve = TRUE)),
         auc = map_dbl(pr, ~ .x$auc.integral), # Extract AUC for each PR curve
         coords = map(pr, ~ data.frame(recall = .x$curve[, 1], precision = .x$curve[, 2]))) %>%
  select(-data, -pr) %>%
  unnest(cols = c(coords)) %>%
  ungroup()

# Plot the PR curves
compare_models_pr_plot <- ggplot(compare_models_pr_df, aes(x = recall, y = precision, color = model)) +
  geom_line() +
  labs(title = "PR Curves by Model and Subsample ID",
       x = "Recall (Sensitivity)",
       y = "Precision",
       color = "Model") +
  theme_minimal()

print(compare_models_pr_plot)
