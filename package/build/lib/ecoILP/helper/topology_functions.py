...
# @log_time(should_log=False)
# def calculate_symbiotic_relationships(B, top_nodes, bottom_nodes, **kwargs):
#     Map community to a corresponding symbiotic relationship
#     symbiotic_relationships = { 
#             "Plant-Pollinator":"Mutualism",
#             "Plant-Ant":"Mutualism",
#             "Plant-Seed Dispersers":"Mutualism",
#             "Anemone-Fish":"Mutualism",
#             #"":"Commensalism",
#             "Host-Parasite":"Antagonism",
#             "Plant-Herbivore":"Antagonism",
#             "Plant-Frugivore":"?",
#             "Plant–Vertebrate":"?",
#             "Multiples":"Multiples",
#             }
...
def infomap_helper(B, u=None, v=None, return_df=False):
    ...
    # start_time = time.time()
    if u and v:
        # modules_dict = im.get_modules() # better but doesn't provide the flow
        # int(modules_dict[u_id]==modules_dict[v_id]))
...
@log_time(should_log=False)
def calculate_node2vec(B, top_nodes, bottom_nodes, **kwargs):
    # edges, non_edges = kwargs.get('edges'), kwargs.get('non_edges')
    # return bridges_helper(B, bottom_nodes, non_edges)    
    g_emb = n2v(B, dimensions=64, quiet=True)
    ...
    emb_df = (
    pd.DataFrame(
        [mdl.wv.get_vector(str(n)) for n in B.nodes()],
        index = B.nodes
        )
    )
    # print(emb_df)
...
def dict2df(network, features_dict, top_nodes, bottom_nodes):
    ...
    # Add node embeddings to edge list
    if 'node_embedding' in features_dict:
        node_embedding = pd.concat([feature_df.add_suffix('_'+feature_name) for feature_name, feature_df in features_dict["node_embedding"].items()], axis=1)
        network = network.merge(node_embedding, how='left', left_on='lower_level', right_index=True)
...
def extract_embeddings(df, features_list, nslots=1, table_path='data/processed/features/embeddings_py.csv'):

    # Define a dictionary of all features and their corresponding functions
    feature_functions = {
        'Node2Vec': (calculate_node2vec, 'node_embedding'),
    }

    # list_of_dfs = list(dict(tuple(df.groupby('subsample_ID'))).values())

    # # Convert the edgelists to a networkx graph
    # Bs = [pandas2bigraph(edgelist) for edgelist in list_of_dfs]

    # # Function to relabel nodes with a unique identifier
    # def relabel_nodes(graph, prefix):
    #     mapping = {node: f"{prefix}_{node}" for node in graph.nodes()}
    #     return nx.relabel_nodes(graph, mapping)

    # Bs_relabelled = [relabel_nodes(B, index) for index, B in enumerate(Bs)]

    # # Now compose all the graphs
    # B = nx.compose_all(Bs_relabelled)

    # Rename df's 'higher_level' and 'lower_level' values with a prefix of the coresponding subsample_ID
    df['higher_level'] = df['subsample_ID'].astype(str) + '_' + df['higher_level'].astype(str)
    df['lower_level'] = df['subsample_ID'].astype(str) + '_' + df['lower_level'].astype(str)
    ...
    features_dict = {'node_embedding': {}}
    
    for feature in features_list:
        if not (feature in feature_functions):
            raise Exception(f"Feature {feature} not found in feature_functions")
        ...
        # Call the appropriate function and store the result
        feature_result = func(B, top_nodes, bottom_nodes)

        features_dict[level][feature] = feature_result
        
    # Convert the dictionary to a dataframe
    embeddings_df = dict2df(df, features_dict, top_nodes, bottom_nodes)
    
    embeddings_df.to_csv(table_path, index=False, mode='a', header=not pd.io.common.file_exists(table_path))
